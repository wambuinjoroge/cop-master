    ...
    <section class="content">
      ...
      <div class="box box-default">
        <div class="box-header with-border">
         <center><h3 class="box-title">Add a new role</h3></center> 
          ...
          <form action="{{ route('roles.store') }}" method="POST">
                       ...
                       <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
                           <label>Name (Pick your choice carefully, this can't be edited later once saved)</label> 
                            <input type="text" placeholder="eg admin" value="{{ old('name') }}" name="name" class="form-control" minlength="3" maxlength="15" required>
                       ...
                       <div class="form-group{{ $errors->has('display_name') ? ' has-error' : '' }}">
                           <label>Display Name</label> 
                            <input type="text" placeholder="eg System Administrator" value="{{ old('display_name') }}" name="display_name" class="form-control" minlength="5" maxlength="30" required>
                            <!-- If role name has error -->
                       ...
                       <div class="form-group{{ $errors->has('description') ? ' has-error' : '' }}">
                           <label>Description</label> 
                            <input type="text" placeholder="eg User can manage all users" value="{{ old('description') }}" name="description" class="form-control" required>
                            ...
                            @foreach($permissions->chunk(3) as $chunk)
                                <div class="row">
                                    @foreach($chunk as $permission)
                                        <div class="col-xs-4">
                                            <input type="checkbox" name="permissions[]" value="{{ $permission->id }}" minlength="5"> {{ $permission->name }} <br>
                                        </div>
                                    @endforeach
                                </div>
                            @endforeach
